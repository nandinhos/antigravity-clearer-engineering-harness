...
6. **Identificação Prévia de Ambiente & Rigores Granulares (2026-09-06)**: O harness passou a identificar compulsoriamente o ambiente alvo (`DEV`, `HOMOLOGACAO` ou `PRODUCAO`) antes de qualquer mutação. Em `DEV`, destrutivos são permitidos para correção ágil (`ALLOW` com salvaguardas); em `HOMOLOGACAO`, exigem dupla confirmação (`ASK` com 2 alertas); em `PRODUCAO`, são sumariamente bloqueados (`DENY`).
7. **Topologia Flexível de Branches & Assistente de Configuração (2026-09-06)**: Suporte nativo aos dois modos canônicos de trabalho: Modo Enterprise (3 branches: `dev` -> `staging` -> `main`) e Modo Clássico (2 branches: `dev` -> `main`). O script `detect-project.sh` audita a topologia na inicialização e o utilitário `setup-branches.sh` permite configurar o repositório em 1 comando.
