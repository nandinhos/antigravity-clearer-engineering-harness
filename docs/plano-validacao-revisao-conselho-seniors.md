...
**Versão:** 0.25.0
**Estado:** Etapa 3 concluída (Cluster 1 blindado sob o novo contrato minimalista do Handoff 003, P0 de config ignorada resolvido, T1–T6 validados em 38 cenários determinísticos; Aguardando homologação do owner para o Cluster 2).
...
**Status atual do Cluster 1 (P0):** O contrato minimalista do Handoff 003 está integralmente implementado no branch `fix/cluster1-contrato-ci`. O bypass P0 de configuração ignorada foi neutralizado e testado (T6). As decisões D1 e D2 foram consolidadas. A suíte de aceitação permanente (`cluster1_acceptance.py`) passa **38/38 cenários (100%)**, a suíte canônica do CEH (`run-all-tests.sh`) passa **42/42 testes (100%)** e os smoke-evals (`evals/run.sh`) passam **5/5 (100%)**. Orçamento de linhas cumprido com folga. Zero `git push` executado. Próximo passo: homologação formal do owner para fechamento do Cluster 1 e desbloqueio do Cluster 2 (R6, R7, R8).
