...
| `Inspeção estática` | O fluxo é indicado pela leitura do código, sem reprodução empírica. | Referência de arquivo/linha e raciocínio. |
| `Reproduzido` | O procedimento isolado demonstrou o comportamento do defeito. | Comando exato, ambiente, saída, exit code e artefato relevante. |
| `Refutado` | A reprodução empírica contrariou a hipótese do defeito. | Mesmo conjunto de evidências de `Reproduzido`, incluindo a divergência. |
...
| `Reproduzido e corrigido` | O defeito foi reproduzido, a correção cirúrgica foi implementada no código e validada por teste automatizado permanente com evidência pós-fixação. | Teste de regressão permanente (`OBSERVED`), exit code 0 comprovado e artefato postfix correspondente. |
...
Esta tabela registra estado, evidências e resolução técnica dos achados; não substitui uma ata de aprovação do conselho. “Implementação consolidada” resume a solução registrada no código, e “Commit de implementação” aponta o commit local correspondente. A existência de um commit, por si só, não significa homologação formal.

| ID | Estado atual | Evidência conferida | Critério/decisão registrado | Severidade/prioridade registrada | Implementação consolidada | Commit de implementação |
|---|---|---|---|---|---|---|
| R1 | Reproduzido e corrigido | [`r1_compound_commands_evidence.json`](temp_implementation/evidence/r1_compound_commands_evidence.json), [`r1_cluster1_postfix.json`](temp_implementation/evidence/r1_cluster1_postfix.json) | Decomposição formal via Lexer FSM com suporte a `&` isolado; normalização desaspeada (quote removal) contra ofuscação (`ph''p`); Fail-Closed incondicional para ANSI-C quoting (`$'...'`), `$()`, backticks e subshells; precedência `DENY > ASK > ALLOW`. | Alta / P0 | Decomposição léxica FSM implementada no commit `bb61fe7` (Handoff 002 e ADR 004). | `bb61fe7` |
| R2 | Reproduzido e corrigido | [`r2_ci_certificate_evidence.json`](temp_implementation/evidence/r2_ci_certificate_evidence.json), [`r2_cluster1_postfix.json`](temp_implementation/evidence/r2_cluster1_postfix.json) | Canonicidade por igualdade estrita (`RAW_TEST_CMD == CANONICAL_CMD`), certificado condicionado a worktree limpo, `.ceh/config.json` obrigatoriamente rastreado em HEAD e sem modificações (T6), e serialização via `json.dump`. | Alta / P0 | Contrato cirúrgico Ponytail implementado no commit `bb61fe7` (Handoff 003 e ADR 004). | `bb61fe7` |
| R3 | Reproduzido e corrigido | [`r3_uninstall_aliases_evidence.json`](temp_implementation/evidence/r3_uninstall_aliases_evidence.json), [`r3_cluster3_postfix.json`](temp_implementation/evidence/r3_cluster3_postfix.json) | Instalação e desinstalação devem usar bloco delimitado exclusivo para remoção atômica de todos os aliases do CEH, com guarda em `install.sh`. | Alta / P2 | Bloco canônico delimitado `# BEGIN/END CLEARER ENGINEERING HARNESS (CEH) ALIASES` via Python inline no commit `b7df47d`. | `b7df47d` |
| R4 | Reproduzido e corrigido | [`r4_review_diff_evidence.json`](temp_implementation/evidence/r4_review_diff_evidence.json), [`r4_cluster3_postfix.json`](temp_implementation/evidence/r4_cluster3_postfix.json) | Contrato de revisão deve exigir e explicitar inspeção do tripé: working tree, staging e commit base. | Alta / P2 | Tripé canônico formalizado em `clearer-review` no commit `b7df47d`. | `b7df47d` |
| R5 | Reproduzido e corrigido | [`r5_git_c_flag_evidence.json`](temp_implementation/evidence/r5_git_c_flag_evidence.json), [`r5_cluster1_postfix.json`](temp_implementation/evidence/r5_cluster1_postfix.json) | Resolução canônica cumulativa de `-C <path>` e caminhos com espaços em `resolve_git_invocation()` com `find_repo_root()`; remoção da isenção de força (gate de CI em todo push, incluindo `-f` e `+ref`); Fail-Closed para opções não homologadas. | Média / P0 | Resolução cumulativa e gate universal de push implementados no commit `bb61fe7` (Handoff 003 e ADR 004). | `bb61fe7` |
| R6 | Reproduzido e corrigido | [`r6_pipeline_exit_evidence.json`](temp_implementation/evidence/r6_pipeline_exit_evidence.json), [`r6_cluster2_postfix.json`](temp_implementation/evidence/r6_cluster2_postfix.json) | `run_test` só aprova o teste de falha quando o runner retorna não-zero **e** a saída contém `STATUS: FAIL`; o exit code não é ocultado por `grep`. | Média / P1 | Asserção combinada de exit code não-zero e marcador FAIL implementada no commit `1c9a0d2`. | `1c9a0d2` |
| R7 | Reproduzido e corrigido | [`r7_eval_deriva_b_evidence.json`](temp_implementation/evidence/r7_eval_deriva_b_evidence.json), [`r7_cluster2_postfix.json`](temp_implementation/evidence/r7_cluster2_postfix.json) | Deriva B exige mutação não vazia, Python compilável, controle original `deny/production/2` e mutante `allow/development/0`; erro de infraestrutura não conta como detecção. | Média / P1 | Validação de diff, `py_compile` e resultado semântico implementada no commit `1c9a0d2`. | `1c9a0d2` |
| R8 | Reproduzido e corrigido | [`r8_eval_dirty_repo_evidence.json`](temp_implementation/evidence/r8_eval_dirty_repo_evidence.json), [`r8_cluster2_postfix.json`](temp_implementation/evidence/r8_cluster2_postfix.json) | `evals/run.sh` exige baseline Git limpa antes de iniciar a avaliação e não altera o estado sujo encontrado. | Média / P1 | Verificação antecipada de repositório limpo (`git status --porcelain`) implementada no commit `1c9a0d2`. | `1c9a0d2` |
| R9 | Reproduzido e corrigido | [`r9_sed_portability_evidence.json`](temp_implementation/evidence/r9_sed_portability_evidence.json), [`r9_cluster3_postfix.json`](temp_implementation/evidence/r9_cluster3_postfix.json) | Eliminar dependência de especificidades do `sed` de plataforma; usar substituição portável agnóstica de SO. | Média / P2 | Substituição de `sed -i` por script Python inline em `install.sh` e `uninstall.sh` no commit `b7df47d`. | `b7df47d` |
| R10 | Reproduzido e corrigido | [`r10_doc_link_evidence.json`](temp_implementation/evidence/r10_doc_link_evidence.json), [`r10_cluster3_postfix.json`](temp_implementation/evidence/r10_cluster3_postfix.json) | Substituir caminhos absolutos locais por links relativos portáveis no repositório. | Baixa / P3 | Substituição de referências absolutas locais por links relativos na documentação no commit `b7df47d`. | `b7df47d` |
...
| ID | Estado pré-fix | Observado | Relação | Resolução técnica (Handoff 003 / ADR 004) |
|---|---|---|---|---|
| G1 | Reproduzido | Certificado escrito à mão com `echo` → `allow` | R2 | Fora do modelo (exige intenção adversária; autoridade é o CI remoto) |
| G2 | Reproduzido | HEAD quebrado com correção só no worktree → certificado do HEAD e `allow` | R2 | Corrigido: runner só emite certificado com worktree 100% limpo |
| G3 | Reproduzido | `git push -f`, `--force-with-lease` e `+ref` sem certificado em development → `allow` | R5 | Corrigido: gate de CI compulsório em todo push, sem isenção de força |
| G4 | Reproduzido | Comando com aspas → certificado com JSON inválido | R2 | Corrigido: gravação atômica via `python3 -c "import json; json.dump(...)"` |
| G5 | Reproduzido | `"test": "python3 -Ic 'exit(0)'"` → `canonical_verified: true` e `allow` | R2 | Fora do modelo (exige intenção adversária; autoridade é o CI remoto) |
...
| 0.28.0 | 2026-09-23 | Reconciliação do registro R1–R10 para distinguir resolução implementada de homologação formal; atualização da suíte geral para 45/45; escopo do `doc-audit` limitado explicitamente a sete verificações estruturais, com checagem dos cabeçalhos do registro e sem alegação de auditoria semântica integral. | Plano reconciliado; aprovação formal do conselho continua sendo decisão humana. |
...
**Estado de validação:** O Cluster 1 foi concluído no commit local `bb61fe7` (38 cenários); o Cluster 2 (R6–R8), no commit `1c9a0d2`; e o Cluster 3 (R3, R4, R9, R10), com aceitação 3/3. A suíte geral atual foi executada com 45/45 PASS, e os smoke-evals com 5/5 PASS. Os achados R1–R10 têm evidências de correção e testes permanentes conforme os registros acima. A rotina `doc-audit` verifica sete propriedades estruturais enumeradas no seu relatório; ela não certifica coerência semântica integral nem aprovação formal do conselho.

**Homologação Formal do Conselho:** Em sessão de 2026-09-23, o Conselho de Seniores concluiu formalmente a Etapa 5 ("Verificar e fechar") e emitiu a ata de homologação unânime ([Handoff 004](temp_implementation/handoffs/handoff-004-homologacao-conselho-seniores.md)), chancelando como **HOMOLOGADOS INTEGRALMENTE** todos os 10 achados (R1–R10), os contratos de CI (G1–G5) e a infraestrutura de auditoria estrutural (`doc-audit`), recomendando a promoção da branch `fix/cluster1-contrato-ci` para `dev`.
