...
**Versão:** 0.24.0  
**Estado:** Etapa 3 reaberta para R2/R5. Houve implementação antes da aprovação (diff não commitado sobre `7da7ae2`), e o probe G1–G5 reproduziu lacunas de contrato que os 58 cenários não cobriam. O redesenho está no [Handoff 003](temp_implementation/handoffs/handoff-003-redesenho-contrato-ci.md) e aguarda as decisões D1–D2 do owner. O Cluster 2 continua bloqueado.  
...
| R3 | Reproduzido | [`r3_uninstall_aliases_evidence.json`](temp_implementation/evidence/r3_uninstall_aliases_evidence.json) | Instalação e desinstalação devem usar bloco delimitado exclusivo para remoção atômica de todos os aliases do CEH. | Alta / P2 | Bloco canônico `# BEGIN CEH ALIASES` / `# END CEH ALIASES` no `.bashrc`/`.zshrc`. | Pendente (aprovador: owner) |
| R4 | Reproduzido | [`r4_review_diff_evidence.json`](temp_implementation/evidence/r4_review_diff_evidence.json) | Contrato de revisão deve exigir e explicitar inspeção do tripé: working tree, staging e commit base. | Alta / P2 | Instruir `git diff`, `git diff --cached` e comparação com branch upstream ou último commit. | Pendente (aprovador: owner) |
...
| R6 | Reproduzido | [`r6_pipeline_exit_evidence.json`](temp_implementation/evidence/r6_pipeline_exit_evidence.json) | Asserções de teste devem capturar explicitamente o status do runner antes do pipe ou ativar `pipefail`. | Média / P1 | Habilitar `set -o pipefail` e avaliar `${PIPESTATUS[0]}` em asserções de erro do test runner. | Pendente (aprovador: owner) |
| R7 | Reproduzido | [`r7_eval_deriva_b_evidence.json`](temp_implementation/evidence/r7_eval_deriva_b_evidence.json) | A Deriva B só aprova se a mutação semântica for compilável e resultar na decisão relaxada esperada (`allow`). | Média / P1 | Testar sintaxe com `python3 -m py_compile` e checar JSON retornado para descartar `SyntaxError`. | Pendente (aprovador: owner) |
| R8 | Reproduzido | [`r8_eval_dirty_repo_evidence.json`](temp_implementation/evidence/r8_eval_dirty_repo_evidence.json) | Alinhar contrato normativo e código: runner de eval deve exigir baseline 100% limpa antes da execução. | Média / P1 | Abortar no início com `fail-early` se `git status --porcelain` contiver arquivos pendentes. | Pendente (aprovador: owner) |
| R9 | Inconclusivo | [`r9_sed_portability_evidence.json`](temp_implementation/evidence/r9_sed_portability_evidence.json) | Eliminar dependência de especificidades do `sed` de plataforma; usar substituição portável agnóstica de SO. | Média / P2 | Substituir `sed -i` por script Python inline portável ou helper com suporte a GNU e BSD sed. | Pendente (aprovador: owner) |
| R10 | Reproduzido | [`r10_doc_link_evidence.json`](temp_implementation/evidence/r10_doc_link_evidence.json) | Substituir caminhos absolutos locais por links relativos portáveis no repositório. | Baixa / P3 | Usar `../clearer-engineering/scripts/safety-gate.py` no markdown da documentação. | Pendente (aprovador: owner) |

### Achados pós-implementação do Cluster 1 (G1–G5)

Reproduzidos em 2026-09-23T11:46-03:00, com o CEH em HEAD `7da7ae2` mais o diff não commitado, em Linux 6.18 (WSL2). Procedimento: [`probe_cluster1_contract_gaps.sh`](temp_implementation/scripts/probe_cluster1_contract_gaps.sh). Saída: [`cluster1_contract_gaps_probe.txt`](temp_implementation/evidence/cluster1_contract_gaps_probe.txt). A análise está no [Handoff 003](temp_implementation/handoffs/handoff-003-redesenho-contrato-ci.md), seção 2.
...
**Status atual do Cluster 1 (P0):** R1 segue válido. R2 e R5 (force push) foram reabertos. Os números 58/58, 42/42 e 5/5 foram reconferidos em 2026-09-23 e continuam verdes, mas medem só os cenários que existem, e o probe G1–G5 mostrou lacunas fora deles. A implementação v22 continua não commitada e sem aprovação do owner. Próximo passo: o owner responde D1–D2 do [Handoff 003](temp_implementation/handoffs/handoff-003-redesenho-contrato-ci.md), e só depois disso o executor segue a seção 10 daquele documento. O Cluster 2 (R6, R7, R8) continua bloqueado.
